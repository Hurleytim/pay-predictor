...
// ── DUTY TYPE FLAGS ──────────────────────────────────────────────────────────
// countsOT   : increments the OT worked-day counter
// countsSlot : consumes a base-day slot (reduces remaining planned days)
const DUTY_TYPES = [
  { id:"IP",       label:"Instructor of Record",   hasOverride:true,  isCancelled:false, isVac:false, isSick:false, isFly:false, countsOT:true,  countsSlot:true  },
  { id:"SEAT",     label:"Seat Filler",            hasOverride:true,  isCancelled:false, isVac:false, isSick:false, isFly:false, countsOT:true,  countsSlot:true  },
  { id:"SIM",      label:"Simulator Event",        hasOverride:true,  isCancelled:false, isVac:false, isSick:false, isFly:false, countsOT:true,  countsSlot:true  },
  { id:"PT",       label:"Practice Teaching",      hasOverride:true,  isCancelled:false, isVac:false, isSick:false, isFly:false, countsOT:true,  countsSlot:true  },
  { id:"OBS",      label:"Observing",              hasOverride:false, isCancelled:false, isVac:false, isSick:false, isFly:false, countsOT:true,  countsSlot:true  },
  { id:"WDAY",     label:"W Day",                  hasOverride:false, isCancelled:false, isVac:false, isSick:false, isFly:false, countsOT:true,  countsSlot:true  },
  { id:"RECUR",    label:"Recurrent Training",     hasOverride:false, isCancelled:false, isVac:false, isSick:false, isFly:false, countsOT:true,  countsSlot:true  },
  { id:"GROUND",   label:"Ground Training",        hasOverride:false, isCancelled:false, isVac:false, isSick:false, isFly:false, countsOT:true,  countsSlot:true  },
  { id:"SICK",     label:"Sick",                   hasOverride:false, isCancelled:false, isVac:false, isSick:true,  isFly:false, countsOT:false, countsSlot:false },
  { id:"WKCXLD",   label:"Cancelled Work Day",     hasOverride:false, isCancelled:true,  isVac:false, isSick:false, isFly:false, countsOT:false, countsSlot:true  },
  { id:"VAC_WEEK", label:"Vacation Week (7 days)", hasOverride:false, isCancelled:false, isVac:true,  isSick:false, isFly:false, countsOT:false, countsSlot:true  },
  { id:"VAC_DAY",  label:"Vacation Day",           hasOverride:false, isCancelled:false, isVac:true,  isSick:false, isFly:false, countsOT:false, countsSlot:true  },
  { id:"FLY",      label:"Fly Through Hours",      hasOverride:false, isCancelled:false, isVac:false, isSick:false, isFly:true,  countsOT:false, countsSlot:false },
];
...
// ─── CALC SINGLE DAY ─────────────────────────────────────────────────────────
// otCount = how many OT-eligible worked days logged INCLUDING this day
function calcDay(day, otCount) {
  if (!day.active) return { base:0, ot:0, override:0, holiday:0, vac:0, fly:0, total:0 };
  const dt = getDT(day.dutyType);

  if (dt.isVac)       return { base:0, ot:0, override:0, holiday:0, vac:VAC_HOURS_DAY, fly:0, total:VAC_HOURS_DAY };
  if (dt.isFly)       { const fly=day.flyHours||0; return { base:0, ot:0, override:0, holiday:0, vac:0, fly, total:fly }; }
  if (dt.isSick)      return { base:BASE_HOURS, ot:0, override:0, holiday:0, vac:0, fly:0, total:BASE_HOURS };
  if (dt.isCancelled) return { base:BASE_HOURS, ot:0, override:0, holiday:0, vac:0, fly:0, total:BASE_HOURS };

  // Normal worked day
  const base     = BASE_HOURS;
  const autoOT   = otCount > OT_THRESHOLD;
  const isOT     = day.otManual !== null ? day.otManual : autoOT;
  const ot       = isOT ? OT_HOURS : 0;
  const overElig = dt.hasOverride || day.overrideOn;
  const override = overElig ? OVERRIDE_PCT * (base + ot) : 0;
  const holiday  = day.holiday ? HOLIDAY_HOURS : 0;
  return { base, ot, override, holiday, vac:0, fly:0, total:base+ot+override+holiday, isOT, overElig, autoOT };
}

// ─── HYBRID ESTIMATE ─────────────────────────────────────────────────────────
// Remaining slots assumed to be normal worked days
// loggedOTCount  = worked days so far (OT counter)
// loggedSlotCount = base-slot-consuming days so far (worked + cancelled + vac)
function calcHybridBreakdown(loggedTotals, loggedOTCount, loggedSlotCount, baseDays) {
  const remainingSlots = Math.max(0, baseDays - loggedSlotCount);
  let plannedBase = 0;
  let plannedOT   = 0;
  for (let i = 1; i <= remainingSlots; i++) {
    plannedBase += BASE_HOURS;
    if ((loggedOTCount + i) > OT_THRESHOLD) plannedOT += OT_HOURS;
  }
  return {
    base:     loggedTotals.base     + plannedBase,
    ot:       loggedTotals.ot       + plannedOT,
    override: loggedTotals.override,
    holiday:  loggedTotals.holiday,
    vac:      loggedTotals.vac,
    fly:      loggedTotals.fly,
    grand:    loggedTotals.grand    + plannedBase + plannedOT,
  };
}

function calcBaselineTotal(baseDays) {
  let h=0;
  for (let i=1; i<=baseDays; i++) h += (i > OT_THRESHOLD ? BASE_HOURS+OT_HOURS : BASE_HOURS);
  return h;
}
...
function Toggle({ on, onChange, color=T.acc }) {
  return (
    <div onClick={onChange} style={{ width:44,height:26,borderRadius:13,background:on?color:T.border,
...
function Pill({ label, active, onClick, color }) {
  return (
    <button onClick={onClick} style={{ padding:"6px 12px",borderRadius:20,
      ...
      color:active?color:T.muted,fontSize:12,fontFamily:"'Roboto Mono',monospace",
      cursor:"pointer",transition:"all 0.15s",whiteSpace:"nowrap" }}>{label}</button>
  );
}
    ...
    <div style={{ marginTop:4 }}>
      ...
      <div style={{ display:"flex",gap:8,alignItems:"center",flexWrap:"wrap" }}>
        <input type="text" inputMode="decimal"
          placeholder={format==="decimal" ? "e.g. 2.5" : "e.g. 2.30"} value={raw}
      ...
      {value > 0 && (
        <div style={{ marginTop:6,fontSize:11,color:T.teal,fontFamily:"'Roboto Mono',monospace" }}>
          = {fmtH(value)} hrs decimal{format==="hhmm" && raw && ` (${Math.floor(value)}h ${Math.round((value%1)*60)}m)`}
        </div>
      )}
...
function DayRow({ day, calc, otCount, onUpdate }) {
  const dt      = getDT(day.dutyType);
  const isVac   = dt.isVac;
  const isCxld  = dt.isCancelled;
  const isSick  = dt.isSick;
  const isFly   = dt.isFly;
  const isOTDay = dt.countsOT && otCount > OT_THRESHOLD;
  ...
  const borderCol = !day.active ? T.border : isVac?T.purple : (isCxld||isSick)?T.red : isFly?T.teal : isOTDay?T.acc : T.green;
  const rowBg     = !day.active ? "transparent" : isVac?T.purpleL : (isCxld||isSick)?T.redL : isFly?T.tealL : isOTDay?T.accL : T.greenL;
  ...
  return (
    <div style={{ border:`1px solid ${borderCol}`,borderRadius:10,marginBottom:8,background:rowBg,overflow:"hidden",transition:"all 0.2s" }}>
      <div style={{ display:"flex",alignItems:"center",gap:10,padding:"11px 12px" }}>
        <Toggle on={day.active} onChange={()=>upd("active",!day.active)}
          color={isVac?T.purple:isFly?T.teal:isOTDay&&day.active?T.acc:T.green} />
        <div style={{ flex:1,minWidth:0 }}>
          <div style={{ display:"flex",alignItems:"center",gap:6,flexWrap:"wrap" }}>
            ...
            {day.active && isFly  && <span style={{ fontSize:10,color:T.teal,fontFamily:"'Roboto Mono',monospace",background:T.tealL,padding:"2px 6px",borderRadius:4 }}>✈ FLY</span>}
            {day.active && isSick && <span style={{ fontSize:10,color:T.red,fontFamily:"'Roboto Mono',monospace",background:T.redL,padding:"2px 6px",borderRadius:4 }}>SICK</span>}
            {day.active && isCxld && <span style={{ fontSize:10,color:T.red,fontFamily:"'Roboto Mono',monospace",background:T.redL,padding:"2px 6px",borderRadius:4 }}>CXLD</span>}
          </div>
        ...
        {day.active && (
          <div style={{ textAlign:"right",flexShrink:0 }}>
            <div style={{ fontFamily:"'Barlow Condensed',sans-serif",fontWeight:700,fontSize:22,
              color:isVac?T.purple:isFly?T.teal:(isSick||isCxld)?T.red:isOTDay?T.acc:T.green,lineHeight:1 }}>
              {fmtH(calc.total)}
            </div>
          ...
          {dt.countsOT && (
            <div style={{ display:"flex",gap:6,flexWrap:"wrap" }}>
              <Pill label={`15% Override${dt.hasOverride?" (auto)":""}`}
                ...
                onClick={()=>!dt.hasOverride&&upd("overrideOn",!day.overrideOn)} color={T.green} />
              <Pill label={`OT +2.5${autoOT&&day.otManual===null?" (auto)":""}`}
                active={showOT} onClick={()=>upd("otManual",showOT?false:true)} color={T.acc} />
              <Pill label="Holiday +5.25" active={day.holiday}
                onClick={()=>upd("holiday",!day.holiday)} color={T.purple} />
            </div>
          )}
          {isSick  && <div style={{ fontSize:11,color:T.red,fontFamily:"'Roboto Mono',monospace" }}>Sick · 6.00 hrs flat · Does not count toward OT threshold or base days</div>}
          {isCxld  && <div style={{ fontSize:11,color:T.red,fontFamily:"'Roboto Mono',monospace" }}>Cancelled · 6.00 hrs flat · Counts as base day · No OT or override</div>}
          {isFly   && <FlyHoursInput value={day.flyHours} format={day.flyFormat} onChangeValue={v=>upd("flyHours",v)} onChangeFormat={f=>upd("flyFormat",f)} />}
          {isVac && dt.id==="VAC_WEEK" && <div style={{ fontSize:11,color:T.purple,fontFamily:"'Roboto Mono',monospace" }}>✓ Next 7 days auto-filled as vacation</div>}
        </div>
      )}
    </div>
  );
...
function SummaryTab({ hybrid, workDays, fleet, setFleet, baseDays, hasEntries }) {
  const rates  = PAY_RATES[CONTRACT_YEAR];
  const rate   = rates[fleet];
  const gross  = hybrid.grand * rate;

  const rows = [
    { label:"Base Pay Hours",         value:hybrid.base,     color:T.text,   code:"INSTRHRS"   },
    { label:"Overtime (2.5 hrs/day)", value:hybrid.ot,       color:T.acc,    code:"INSTR OT"   },
    { label:"IP Override (15%)",      value:hybrid.override, color:T.green,  code:"INSTR 15"   },
    { label:"Holiday Premium",        value:hybrid.holiday,  color:T.purple, code:"PM HOLIDAY" },
    { label:"Vacation Hours",         value:hybrid.vac,      color:T.blue,   code:"VAC"        },
    { label:"Fly Through Hours",      value:hybrid.fly,      color:T.teal,   code:"FLY"        },
  ];
  ...
  return (
    <div style={{ padding:"16px 16px 60px" }}>
      ...
      <div style={{ background:T.card,border:`1px solid ${T.border}`,borderRadius:10,overflow:"hidden",marginBottom:16 }}>
        <div style={{ padding:"10px 16px",background:T.panel,borderBottom:`1px solid ${T.border}` }}>
          <div style={{ fontSize:10,color:T.muted,textTransform:"uppercase",letterSpacing:2,fontFamily:"'Roboto Mono',monospace" }}>
            Hour Breakdown {!hasEntries&&<span style={{ color:T.acc }}>(baseline)</span>}{hasEntries&&hybrid.ot>0&&<span style={{ color:T.acc }}> · incl. planned OT</span>}
          </div>
        </div>
        {rows.map(r=>(
        ...
        <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",padding:"14px 16px",background:T.accL }}>
          ...
          <div style={{ fontFamily:"'Barlow Condensed',sans-serif",fontWeight:800,fontSize:32,color:T.acc }}>{fmtH(hybrid.grand)}</div>
        </div>
      </div>
      <div style={{ background:T.greenL,border:`1px solid ${T.green}40`,borderRadius:10,padding:"18px 16px" }}>
        ...
        <div style={{ fontSize:11,color:T.muted,fontFamily:"'Roboto Mono',monospace",marginTop:4 }}>{fmtH(hybrid.grand)} hrs × ${rate.toFixed(2)}/hr</div>
      </div>
    </div>
  );
}
...
export default function PayPredictor() {
  ...
  // OT counter increments ONLY for countsOT days (worked days)
  const enriched = useMemo(() => {
    let otCount = 0;
    return days.map(day => {
      const dt = getDT(day.dutyType);
      if (day.active && dt.countsOT) otCount++;
      const calc = calcDay(day, otCount);
      return { day, calc, otCount: (day.active && dt.countsOT) ? otCount : null };
    });
  }, [days]);
  ...
  const loggedTotals = useMemo(() => {
    ...
    enriched.forEach(({day,calc}) => {
      ...
      base+=calc.base; ot+=calc.ot; override+=calc.override;
      holiday+=calc.holiday; vac+=calc.vac; fly+=calc.fly;
    });
    return { base, ot, override, holiday, vac, fly, grand:base+ot+override+holiday+vac+fly };
  }, [enriched]);

  const loggedOTCount   = enriched.filter(e=>e.day.active && getDT(e.day.dutyType).countsOT).length;
  const loggedSlotCount = enriched.filter(e=>e.day.active && getDT(e.day.dutyType).countsSlot).length;
  const hasEntries      = enriched.some(e=>e.day.active);
  const baselineHrs     = useMemo(()=>calcBaselineTotal(baseDays),[baseDays]);
  const rate            = PAY_RATES[CONTRACT_YEAR][fleet];

  const hybrid = useMemo(
    () => calcHybridBreakdown(loggedTotals, loggedOTCount, loggedSlotCount, baseDays),
    [loggedTotals, loggedOTCount, loggedSlotCount, baseDays]
  );

  const grossPay    = hybrid.grand * rate;
      ...
      <div style={{ minHeight:"100vh",background:T.bg,maxWidth:480,margin:"0 auto" }}>
          ...
          <div style={{ padding:"16px 16px 60px",animation:"fadeIn 0.2s ease" }}>
                ...
                {[["START DATE",startDate,handleStart],["END DATE",endDate,handleEnd]].map(([lbl,val,fn])=>(
                  <div key={lbl}>
                    ...
                    <input type="date" value={val} onChange={e=>fn(e.target.value)}
                      style={{ width:"100%",background:T.bg,border:`1px solid ${T.border}`,borderRadius:6,padding:"9px 10px",color:T.text,fontSize:13,fontFamily:"'Roboto Mono',monospace" }} />
                  </div>
                ))}
              </div>
            </div>
            <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:14 }}>
              <div style={{ background:T.card,border:`1px solid ${T.border}`,borderRadius:10,padding:"12px 14px" }}>
                ...
                <select value={baseDays} onChange={e=>setBaseDays(Number(e.target.value))}
                  style={{ width:"100%",background:T.bg,border:`1px solid ${T.border}`,borderRadius:6,padding:"9px 10px",color:T.acc,fontSize:17,fontFamily:"'Barlow Condensed',sans-serif",fontWeight:700,outline:"none" }}>
            ...
            <div style={{ background:hasEntries?T.greenL:T.accL,border:`1px solid ${hasEntries?T.green+"40":T.acc+"40"}`,borderRadius:8,padding:"10px 14px",marginBottom:16,display:"flex",justifyContent:"space-between",alignItems:"center" }}>
              <div style={{ fontSize:11,color:hasEntries?T.green:T.acc,fontFamily:"'Roboto Mono',monospace" }}>
                {loggedOTCount>0
                  ? `${loggedOTCount} worked + ${Math.max(0,baseDays-loggedSlotCount)} remaining · ${fmtH(hybrid.grand)} hrs`
                  : `Baseline · ${baseDays} days · ${fmtH(baselineHrs)} hrs`}
              </div>
            ...
            {days.length===0 ? (
              ...
              enriched.map(({day,calc,otCount})=>(
                <DayRow key={day.date} day={day} calc={calc} otCount={otCount} onUpdate={updateDay} />
              ))
            )}
          </div>
        )}
        {tab==="summary" && (
          <div style={{ animation:"fadeIn 0.2s ease" }}>
            <SummaryTab hybrid={hybrid} workDays={loggedOTCount} fleet={fleet} setFleet={setFleet} baseDays={baseDays} hasEntries={hasEntries} />
          </div>
        )}
      </div>
    </>
